          ...
          *)
          ...
          | Alias, Lprim {primitive = Poffsetint offset; args =  [Lvar _ as matcher ]},                    
            Lifthenelse 
             (Lprim 
              { primitive = Pisout; 
                args = [Lconst (Const_int range) as rn; Lvar switcher2];
                loc
                }
               ,
              ifso, 
              Lswitch (Lvar switcher3 ,
               ...
               Ident.same switcher3 id &&
               Ident.same switcher2 id && 
               not (Lam_hit.hit_variable id ifso ) && 
               not (Ext_list.exists_snd sw_consts (Lam_hit.hit_variable id))
             -> 
             Lam.if_  
               (Lam.prim ~primitive:Pisout 
                  ~args:[rn ; 
                         Lam.prim ~primitive:(Poffsetint offset) ~args:[matcher] loc
                        ] loc) ifso  
               (Lam.switch matcher 
                  {px with 
                   sw_consts = 
